...
// Each game is listed with its ID number(like the 11 in Game 11: ...) followed by a semicolon
// - separated list of subsets of cubes that were revealed from the bag(like 3 red, 5 green, 4 blue).
...
const games = input.split('\n').map((line) => {
	const [idPart, ...setsParts] = line.split(': ');
	const id = Number(idPart.split(' ')[1]);
	const sets = setsParts
		.join(': ')
		.split('; ')
		.map((setPart) => {
			const cubes = setPart.split(', ').map((cubePart) => {
				const [count, color] = cubePart.split(' ');
...
for (const game of games) {
	...
	if (isPossible) sumOfPossibleGameIds += game.id;
}
